...
public class Pipe extends PipelineElement implements Tickable {
    ...
    /**
     * Referencia a példány megjelenítéséért felelős View példányra.
     */
    ...
    public static void setup(Object obj, ArrayList<String> params) {
        ...
        ((Pipe) obj).addView(new PipeView((Pipe) obj));
    }

    /**
     * Getter a view adattaghoz.
     * @return pipeWiew
     */
    @Override
    ...
    /**
     * Hozzáad egy view objektumot.
     * @param sw új view
     */
    public void addView(SpriteView sw) {
        pipeView = (PipeView) sw;
    }
    ...
    public int propagateWater(PipelineElement from, int qty) {
        if (isDamaged) {
            ...
            PipelineElement temp = this.getOtherNeighbor(from);
     ...
     * függvénye ezt a metódust. Ha a fixedTime attribútum értéke 0, akkor saját magán meghívja a
