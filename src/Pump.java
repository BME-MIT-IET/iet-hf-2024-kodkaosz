...
public class Pump extends PipelineElement implements Tickable {
    ...
    public List<Object> state() {
        ...
        Object state[] = new Object[] { isDamaged, toPipeIdx, fromPipeIdx };
    ...
    public static void setup(Object obj, ArrayList<String> options) {
        ...
        // view setup
        PumpView pumpView = new PumpView(Integer.parseInt(options.get(options.size() - 2)),
                Integer.parseInt(options.get(options.size() - 1)), (Pump)obj);
        ((Pump) obj).setView(pumpView);
        ((Pump) obj).addView(pumpView);
    }
    ...
    public void setFromPipe(int index) {
        ...
        pumpView.update();
        for(PipelineElement element : neighbors) {

            ((Pipe)element).getView().update();
        }
    }
    ...
    public void setView(SpriteView view) {
        ...
        this.pumpView = (PumpView)view;
    }

    public PipelineElement getFromPipe() {

        return neighbors.get(fromPipeIdx);
    }

    public PipelineElement getToPipe() {

        return neighbors.get(toPipeIdx);
    }
}
