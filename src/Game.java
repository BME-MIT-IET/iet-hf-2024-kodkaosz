...
/**
 * A játék irányításáért felelős osztály. Ebbe beleértjük a játék indítását és az inicializálását, illetve a
 * pontszámok nyilvántartását. Csak egyszer példányosítható, hiszen egyszerre csak egy játékot lehet játszani.
 * Az összes osztály ismeri.
 */
public class Game {
    /**
    ...
    /**
     * A szerelők pontszámának grafikus
     * megjelenítéséért felelős, a Game meghívja az update függvényét, amikor a
     * pontszámban változás van.
     */
    private ScoreCounter plumberScoreCounter = new ScoreCounter("Plumber");

    /**
     * A szabotőrök pontszámának grafikus
     * megjelenítéséért felelős, a Game meghívja az update függvényét, amikor a
     * pontszámban változás van.
     */
    private ScoreCounter saboteurScoreCounter = new ScoreCounter("Saboteur");
    ...
    /**
     * A paraméterként kapott mennyiséget hozzáadja a lostWater attribútumhoz. Az update() függvénye
     * meghívásával frissíti a saboteurScoreCountert.
     * @param qty a mennyiség, amennyit hozzá szeretnénk adni
     */
    public void addLostWater(int qty) {
        setLostWater(lostWater + qty);
        saboteurScoreCounter.setScore(lostWater);
        saboteurScoreCounter.update();
    }

    /**
     * A paraméterként kapott értéket hozzáadja a finishedWater tagváltozóhoz. Az update() függvénye
     * meghívásával frissíti a plumberScoreCountert.
     * @param qty a mennyiség, amennyit hozzá szeretnénk adni
     */
    public void addFinishedWater(int qty) {
        setFinishedWater(finishedWater + qty);
        plumberScoreCounter.setScore(finishedWater);
        plumberScoreCounter.update();
    }

    /**
     * A paraméterként kapott értéket levonja a finishedWater tagváltozóból. 0 alá nem csökkentheti.
     * Az update() függvénye meghívásával frissíti a plumberScoreCountert.
    ...
    public void setPlumberScoreCounter(ScoreCounter plumberScoreCounter) {
        this.plumberScoreCounter = plumberScoreCounter;
    }

    public void setSaboteurScoreCounter(ScoreCounter saboteurScoreCounter) {
        this.saboteurScoreCounter = saboteurScoreCounter;
    }
